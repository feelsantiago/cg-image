...
export class FilterService {
    ...
    public transform(image: PgmFile, mask: Mask, type: MaskType = MaskType.correlation): number[] {
        const newImage = [];

        if (type === MaskType.convolution)
            mask = this.convolutionMask(mask);
    ...
    private getNeighborhoods(index: number, image: PgmFile): Mask {
        ...
        const [x, y] = this.imageHelperService.calculateCoordinates(index, image.width, image.height);
